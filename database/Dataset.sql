...
BEGIN
    ...
    DECLARE brand ENUM('Van Huesen', 'Levi', 'Nike', 'Adidas','H&M','Zara','Zudio','Snitch');
    DECLARE color ENUM('Red', 'Blue', 'Black', 'White','Maroon','Grey');
    DECLARE size ENUM('XS', 'S', 'M', 'L', 'XL','XXL','XXXL');
    DECLARE price INT;
    DECLARE stock INT;
    ...
    WHILE counter < max_records DO
        ...
        SET brand = ELT(FLOOR(1 + RAND() * 8), 'Van Huesen', 'Levi', 'Nike', 'Adidas','H&M','Zara','Zudio','Snitch');
        SET color = ELT(FLOOR(1 + RAND() * 6), 'Red', 'Blue', 'Black', 'White','Maroon','Grey');
        SET size = ELT(FLOOR(1 + RAND() * 7), 'XS', 'S', 'M', 'L', 'XL','XXL','XXXL');
